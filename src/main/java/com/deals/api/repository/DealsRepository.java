...
//	@Query(value = "SELECT * FROM deals d WHERE d.deals_exp >= now()", nativeQuery = true)
	@Query(value = "SELECT * FROM deals WHERE deals.id_deals NOT IN (SELECT d.id_deals FROM deals d LEFT JOIN (SELECT id_deals, COUNT(id_record_deals) as jumlah FROM `deals_record` WHERE id_customer=3 GROUP BY id_deals) J ON d.id_deals = j.id_deals WHERE j.jumlah>=d.deals_max_use) AND deals.deals_exp >= now() ORDER BY (latitude-:latitude)*(latitude-:latitude)+(longitude-:longitude)*(longitude-:longitude)", nativeQuery = true)
    List<Deals> findAvailableDeals(@Param("latitude") float latitude, @Param("longitude") float longitude);
    ...
    Deals checkDeals(@Param("id_deals") int id_deals, @Param("id_customer") int id_customer);
